...
class ConcreteVisitor1 : public Visitor {
 public:
  int visit(Pegas* element) override {
    // std::cout << "VisitPegas\n";
    return 1;
  }
...
void Pegas::accept(std::shared_ptr<NPC> attacker, Visitor& visitor) {
  int res = 0;
...
TEST(test_01, pegas_constructor) {
  int x{100};
  int y{100};
  std::string name{"Pegas_1"};

  std::shared_ptr<NPC> a = std::make_shared<Pegas>(x, y, name);
...
TEST(test_11, on_fight) {
  ...
  int x2{101};
  int y2{101};
  std::string name2{"Pegas_1"};

  std::shared_ptr<NPC> defender = std::make_shared<Pegas>(x2, y2, name2);
...
TEST(test_16, on_fight) {
  ...
  int x1{100};
  int y1{100};
  std::string name1{"Pegas_1"};

  std::shared_ptr<NPC> attacker = std::make_shared<Pegas>(x1, y1, name1);

  int x2{101};
  int y2{101};
  std::string name2{"Pegas_2"};
